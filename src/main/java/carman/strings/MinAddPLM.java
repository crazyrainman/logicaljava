...
public class MinAddPLM {
    ...
    public String getPalindrome2(String str, String strlps) {
        ...
        while(lpsl <= lpsr) {
            ...
            res[resl++] = chas[chasl++]; // from strlps 
            res[resr--] = chas[chasr--];
            lpsl++;
            lpsr--;
        }
    ...
    public void set(char[] res, int resl, int resr, char[] chas, int ls, int le, int rs, int re) {
        for (int i = ls; i < le; i++) {
            res[resl++] = chas[i]; // leftPart
            res[resr--] = chas[i]; // inverse of leftPart
        }
        for (int i = re; i > rs; i--) { 
            res[resl++] = chas[i]; // inverse of rightPart
            res[resr--] = chas[i]; // rightPart
        }
    }
}